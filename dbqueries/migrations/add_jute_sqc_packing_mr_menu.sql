-- Migration: Add Portal menu entry for the new Jute SQC page (Packing MR%)
-- Module: juteSQC (R-08-25 Packing MR% — finished-goods moisture at packing)
-- Date: 2026-06-28
...
--   juteSQC/packingMr  -> Packing MR% SQC (R-08-25)
-- Mirrors add_jute_sqc_bag_check_menu.sql: ALSO grants access -- role_menu_map mirrors the
...
SELECT 'Packing MR% SQC', 'juteSQC/packingMr', 1, 781, NULL, 'inventory_2', 2, 140, NULL
WHERE NOT EXISTS (SELECT 1 FROM menu_mst WHERE menu_path = 'juteSQC/packingMr');
...
WHERE m.menu_path = 'juteSQC/packingMr'
...
-- DELETE r FROM role_menu_map r JOIN menu_mst m ON m.menu_id = r.menu_id WHERE m.menu_path = 'juteSQC/packingMr'
-- DELETE FROM menu_mst WHERE menu_path = 'juteSQC/packingMr'
