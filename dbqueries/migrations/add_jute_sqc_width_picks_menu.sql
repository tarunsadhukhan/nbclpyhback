-- Migration: Add Portal menu entry for the new Jute SQC page (Width and Picks)
-- Module: juteSQC (R-08-21 Width and Picks — on-loom dimensional QC of woven cloth)
-- Date: 2026-06-28
...
--   juteSQC/widthPicks  -> Width and Picks SQC (R-08-21)
-- Mirrors add_jute_sqc_cutting_length_menu.sql: ALSO grants access — role_menu_map mirrors the
...
SELECT 'Width & Picks SQC', 'juteSQC/widthPicks', 1, 781, NULL, 'straighten', 2, 100, NULL
WHERE NOT EXISTS (SELECT 1 FROM menu_mst WHERE menu_path = 'juteSQC/widthPicks');
...
WHERE m.menu_path = 'juteSQC/widthPicks'
...
-- DELETE r FROM role_menu_map r JOIN menu_mst m ON m.menu_id = r.menu_id WHERE m.menu_path = 'juteSQC/widthPicks'
-- DELETE FROM menu_mst WHERE menu_path = 'juteSQC/widthPicks'
