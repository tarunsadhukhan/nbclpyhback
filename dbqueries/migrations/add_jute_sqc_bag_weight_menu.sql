-- Migration: Add Portal menu entry for the new Jute SQC page (Bag Weight)
-- Module: juteSQC (R-08-23 Bag Weight — finished-bag weight control)
-- Date: 2026-06-28
...
--   juteSQC/bagWeight  -> Bag Weight SQC (R-08-23)
-- Mirrors add_jute_sqc_stitch_menu.sql: ALSO grants access — role_menu_map mirrors the
...
SELECT 'Bag Weight SQC', 'juteSQC/bagWeight', 1, 781, NULL, 'shopping_bag', 2, 120, NULL
WHERE NOT EXISTS (SELECT 1 FROM menu_mst WHERE menu_path = 'juteSQC/bagWeight');
...
WHERE m.menu_path = 'juteSQC/bagWeight'
...
-- DELETE r FROM role_menu_map r JOIN menu_mst m ON m.menu_id = r.menu_id WHERE m.menu_path = 'juteSQC/bagWeight'
-- DELETE FROM menu_mst WHERE menu_path = 'juteSQC/bagWeight'
