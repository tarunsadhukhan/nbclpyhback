...
-- role grants are copied from the sibling 'Cash Hands Report' row (the hub row holds
-- only 4 grants, the child reports 15), so whoever sees that report sees this one.
...
FROM menu_mst m
JOIN menu_mst s ON s.menu_path = 'hrms/hrmsreports/cashHands'
