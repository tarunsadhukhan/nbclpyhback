-- Migration: Add Portal menu entry for the new Jute SQC page (Drawhead + Finisher Card)
-- Module: juteSQC (R-08-08/09/10 Drawhead + Finisher Card Sliver Weight)
-- Date: 2026-06-27
...
-- module_mst_id = 2), next to Finisher Drawing (order 70). FE route:
--   juteSQC/drawhead  -> Drawhead SQC (R-08-08/09/10)
...
SELECT 'Drawhead SQC', 'juteSQC/drawhead', 1, 781, NULL, 'view_week', 2, 80, NULL
WHERE NOT EXISTS (SELECT 1 FROM menu_mst WHERE menu_path = 'juteSQC/drawhead');
...
WHERE m.menu_path = 'juteSQC/drawhead'
...
--   WHERE m.menu_path = 'juteSQC/drawhead';
-- DELETE FROM menu_mst WHERE menu_path = 'juteSQC/drawhead';
