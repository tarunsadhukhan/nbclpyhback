-- Migration: Add Portal menu entry for the new Jute SQC page (Stitch)
-- Module: juteSQC (R-08-22 Stitch — finishing-stage sewing-stitch-density QC)
-- Date: 2026-06-28
...
--   juteSQC/stitch  -> Stitch SQC (R-08-22)
-- Mirrors add_jute_sqc_width_picks_menu.sql: ALSO grants access — role_menu_map mirrors the
...
SELECT 'Stitch SQC', 'juteSQC/stitch', 1, 781, NULL, 'content_cut', 2, 110, NULL
WHERE NOT EXISTS (SELECT 1 FROM menu_mst WHERE menu_path = 'juteSQC/stitch');
...
WHERE m.menu_path = 'juteSQC/stitch'
...
-- DELETE r FROM role_menu_map r JOIN menu_mst m ON m.menu_id = r.menu_id WHERE m.menu_path = 'juteSQC/stitch'
-- DELETE FROM menu_mst WHERE menu_path = 'juteSQC/stitch'
