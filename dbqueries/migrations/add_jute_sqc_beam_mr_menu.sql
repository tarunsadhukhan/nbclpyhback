-- Migration: Add Portal menu entry for the new Jute SQC page (Beam MR%)
-- Module: juteSQC (R-08-18 Beam MR% — warp-beam moisture-regain QC)
-- Date: 2026-06-28
...
--   juteSQC/beamMr  -> Beam MR% SQC (R-08-18)
...
SELECT 'Beam MR% SQC', 'juteSQC/beamMr', 1, 781, NULL, 'water_drop', 2, 70, NULL
WHERE NOT EXISTS (SELECT 1 FROM menu_mst WHERE menu_path = 'juteSQC/beamMr');
...
WHERE m.menu_path = 'juteSQC/beamMr'
...
--   WHERE m.menu_path = 'juteSQC/beamMr';
-- DELETE FROM menu_mst WHERE menu_path = 'juteSQC/beamMr';
