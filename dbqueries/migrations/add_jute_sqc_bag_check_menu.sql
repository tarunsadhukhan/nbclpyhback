-- Migration: Add Portal menu entry for the new Jute SQC page (Bag Checking)
-- Module: juteSQC (R-08-24 Bag Checking — finished-bag acceptance inspection)
-- Date: 2026-06-28
...
--   juteSQC/bagCheck  -> Bag Checking SQC (R-08-24)
-- Mirrors add_jute_sqc_bag_weight_menu.sql: ALSO grants access — role_menu_map mirrors the
...
SELECT 'Bag Checking SQC', 'juteSQC/bagCheck', 1, 781, NULL, 'fact_check', 2, 130, NULL
WHERE NOT EXISTS (SELECT 1 FROM menu_mst WHERE menu_path = 'juteSQC/bagCheck');
...
WHERE m.menu_path = 'juteSQC/bagCheck'
...
-- DELETE r FROM role_menu_map r JOIN menu_mst m ON m.menu_id = r.menu_id WHERE m.menu_path = 'juteSQC/bagCheck'
-- DELETE FROM menu_mst WHERE menu_path = 'juteSQC/bagCheck'
