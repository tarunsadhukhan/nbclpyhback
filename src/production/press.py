"""
Press production entries (press_production) — production/pressproduction.

One row per machine + date + shift + quality, straight from the mill's
"PRESS" sheet (Q-CODE, type, prod pcs, rate, amount, MC no, shift, wk hrs,
lost hrs, divisible hrs). The rate is NOT keyed in: it is resolved from the
wages quality master (tbl_nbcl_wages_quality_mst) for the selected quality
and snapshotted on the row; amount = rate * prod_qty, divisible_hrs =
wk_hrs * 4 (beaming uses * 3) and rate_per_hrs = amount / (divisible_hrs -
lost_hrs) are stored generated columns.

Rows are scoped by branch_id (company filter via branch_mst.co_id).
"""
...
from src.models.hrms import PressProduction

router = APIRouter()

# ponytail: the press machines (PM%) and the PRESS sheet's Q-codes both live
# under dept_mst 'FINISHING' in nbcl — there is no separate press dept
_PRESS_DEPT = "FINISHING"


# ─── SQL Queries ──────────────────────────────────────────────────────────


def get_press_prod_list_query():
    return text("""
        SELECT
            p.press_prod_id, p.branch_id, p.prod_date, p.shift,
            p.machine_id, mc.machine_name,
            p.quality_id, q.quality_code, q.quality_desc,
            p.prod_qty, p.rate, p.amount,
            p.wk_hrs, p.lost_hrs, p.divisible_hrs, p.rate_per_hrs,
            p.remarks, p.active
        FROM press_production p
        INNER JOIN branch_mst bm ON bm.branch_id = p.branch_id
        LEFT JOIN machine_mst mc ON mc.machine_id = p.machine_id
        LEFT JOIN tbl_nbcl_wages_quality_mst q ON q.quality_id = p.quality_id
        WHERE p.active = 1
          AND bm.co_id = :co_id
          AND (:branch_id IS NULL OR p.branch_id = :branch_id)
          AND (:from_date IS NULL OR p.prod_date >= :from_date)
          AND (:to_date IS NULL OR p.prod_date <= :to_date)
          AND (:search IS NULL
               OR mc.machine_name LIKE :search
               OR q.quality_code LIKE :search
               OR q.quality_desc LIKE :search)
        ORDER BY p.prod_date DESC, mc.machine_name, p.shift, p.press_prod_id
    """)


def get_press_prod_by_id_query():
    return text("""
        SELECT press_prod_id, branch_id, prod_date, shift, machine_id,
               quality_id, prod_qty, rate, amount, wk_hrs, lost_hrs,
               divisible_hrs, rate_per_hrs, remarks, active
        FROM press_production
        WHERE press_prod_id = :record_id
    """)


def get_press_machines_query():
    return text("""
...
def _duplicate_query():
    ...
    return text("""
        SELECT COUNT(*) AS cnt FROM press_production
        WHERE active = 1
          AND machine_id = :machine_id
          AND prod_date = :prod_date
          AND shift = :shift
          AND quality_id = :quality_id
          AND (:record_id IS NULL OR press_prod_id <> :record_id)
    """)
...
def _parse_body(body: dict) -> dict:
    """Validate + normalise a create/edit payload into PressProduction columns
...
def _resolve_rate(db: Session, quality_id: int) -> float:
    """Press rate as per the wages quality master — never trusted from the client."""
    row = db.execute(get_quality_rate_query(), {"quality_id": quality_id}).fetchone()
    if not row or row.quality_rate is None:
        raise HTTPException(status_code=400, detail="Selected press quality not found")
...
@router.get("/press_prod_setup")
def press_prod_setup(
    request: Request,
    response: Response,
    db: Session = Depends(get_tenant_db),
    token_data: dict = Depends(get_current_user_with_refresh),
):
    """Dropdown options: press machines, shifts and the press quality codes
    with their per-unit rate."""
    try:
        machines = db.execute(get_press_machines_query(), {
            "dept_desc": _PRESS_DEPT,
        }).fetchall()
...
@router.get("/get_press_prod_table")
def get_press_prod_table(
    request: Request,
    response: Response,
    db: Session = Depends(get_tenant_db),
    token_data: dict = Depends(get_current_user_with_refresh),
):
    """Paginated list of press production entries for the selected company/branch."""
    try:
        ...
        rows = db.execute(get_press_prod_list_query(), {
            "co_id": int(co_id),
...
@router.get("/get_press_prod_by_id/{record_id}")
def get_press_prod_by_id(
    record_id: int,
    request: Request,
    response: Response,
    db: Session = Depends(get_tenant_db),
    ...
    try:
        row = db.execute(get_press_prod_by_id_query(), {"record_id": record_id}).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="Press production entry not found")
...
@router.post("/press_prod_create")
def press_prod_create(
    request: Request,
    response: Response,
    db: Session = Depends(get_tenant_db),
    token_data: dict = Depends(get_current_user_with_refresh),
    ...
    try:
        ...
        record = PressProduction(**values, rate=rate, active=1)
        db.add(record)
        db.commit()
        db.refresh(record)
        return {
            "message": "Press production entry created successfully",
            "press_prod_id": record.press_prod_id,
            "rate": record.rate,
...
@router.post("/press_prod_bulk_create")
def press_prod_bulk_create(
    request: Request,
    response: Response,
    db: Session = Depends(get_tenant_db),
    token_data: dict = Depends(get_current_user_with_refresh),
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            rate = _resolve_rate(db, values["quality_id"])
            records.append(PressProduction(**values, rate=rate, active=1))
        ...
        return {
            "message": "Press production entries created successfully",
            "created": len(records),
            "ids": [record.press_prod_id for record in records],
        }
    except HTTPException:
        raise
    except Exception as e:
...
@router.put("/press_prod_edit/{record_id}")
def press_prod_edit(
    record_id: int,
    request: Request,
    response: Response,
    ...
    try:
        values = _parse_body(parse_json_body(request))
        existing = db.query(PressProduction).filter(
            PressProduction.press_prod_id == record_id,
        ).first()
        if not existing:
            raise HTTPException(status_code=404, detail="Press production entry not found")
        ...
        db.commit()
        return {
            "message": "Press production entry updated successfully",
            "press_prod_id": record_id,
        }
    except HTTPException:
        raise
    except Exception as e:
...
@router.delete("/press_prod_delete/{record_id}")
def press_prod_delete(
    record_id: int,
    request: Request,
    response: Response,
    db: Session = Depends(get_tenant_db),
    ...
    try:
        ...
        existing.active = 0
        db.commit()
        return {"message": "Press production entry deleted successfully"}
    except HTTPException:
        raise
    except Exception as e:
        db.rollback()
