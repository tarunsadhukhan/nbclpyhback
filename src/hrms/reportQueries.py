...
def get_employee_face_query():
    ...
    INNER JOIN branch_mst AS bm ON bm.branch_id = pd.branch_id
    -- Some employees carry several active official-details rows (nbcl has
    -- eb_ids with 2-3); take the latest one so each face row appears once.
    LEFT JOIN hrms_ed_official_details AS od
        ON od.tbl_hrms_ed_official_detail_id = (
            SELECT MAX(o2.tbl_hrms_ed_official_detail_id)
            FROM hrms_ed_official_details o2
            WHERE o2.eb_id = pd.eb_id AND o2.active = 1
        )
    LEFT JOIN sub_dept_mst AS sd ON sd.sub_dept_id = od.sub_dept_id
    LEFT JOIN dept_mst AS dm ON dm.dept_id = sd.dept_id
